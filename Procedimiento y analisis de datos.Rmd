...
# 1) Peso de cada combinación Género × Edad
peso_ge <- df_unicos %>%
  count(S2, CuotaEdad) %>%
  rename(N_ge = n) %>%
...
ggplot(pct_ge, aes(x = interaction(CuotaEdad, S2, sep = " \n"), 
  ...
  labs(
    ...
    fill  = "Showrooming",
    title = "Distribución de showrooming por combinación Edad × Género\n(anchura proporcional al tamaño muestral)"
  ) +
  theme_minimal() +
  theme(
...
# 1) Peso de cada combinación Género×Edad×Municipio
...
```{r}
#    - confianza_bin × culpa
#    - servicio_bin × orient_precio
#    - servicio_bin × gratificacion
#    - gratificacion × orient_precio
#    - orient_precio × culpa
...
# Bondad de ajuste del modelo_final
...
# 2) Curva ROC y AUC con ROCR
library(ROCR)
pred_obj  <- prediction(df_clean$prob_final, df_clean$show_bin_num)
perf_obj  <- performance(pred_obj, measure = "tpr", x.measure = "fpr")
plot(
  perf_obj,
  col   = "steelblue",
  lwd   = 2,
  main  = sprintf("ROC del modelo_final (AUC = %.3f)", 
                  performance(pred_obj, "auc")@y.values[[1]])
)
abline(a = 0, b = 1, lty = 2, col = "gray")

# Extraer AUC numérico
...
# 4) Matriz de confusión con gmodels::CrossTable
library(gmodels)
...
# 2) Prepara la matriz de predictores
X <- df_clean[, c(
...
# 3) Calcula valores SHAP (fíjate en object=, no en model=)
set.seed(42)
shap_vals <- explain(
  object       = modelo_final,
  X            = X,
...
# 4) Transforma a data.frame y añade el identificador de caso
shap_df <- as.data.frame(shap_vals)
shap_df$case <- seq_len(nrow(shap_df))

# 5) Pasa a formato “largo” para ggplot
sv_long <- shap_df %>%
  pivot_longer(
    cols      = -case,
    names_to  = "variable",
...
# 6) Dibuja el beeswarm manual
ggplot(sv_long, aes(x = shap_value, y = fct_rev(variable))) +
  geom_violin(fill = "#48b150", alpha = 0.6) +
  geom_jitter(height = 0.1, size = 0.5, alpha = 0.3, color = "#000000") +
  labs(
    x     = "Valor SHAP (contribución al log-odds)",
    y     = "Variable",
    title = "Beeswarm SHAP para modelo_final"
  ) +
  theme_minimal()
